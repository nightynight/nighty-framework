...
public class LoginController {
    ...
    public ResponseEntity login(HttpServletRequest req, HttpServletResponse resp) throws RequestParamException {
        ...
        do {
            ...
            result = OperationResult.buildSuccessResult(new LoginSuccessResult(token,user.getNickname()));
        } while (false);
    ...
    public ResponseEntity isLogin(@RequestParam("username") String username,
        ...
        do {
            ...
            LoginInfo info = securityService.getLoginInfo(user.getUsername());
            if (info == null){
                result = OperationResult.buildFailureResult("还未登录，或登录信息已过期");
                break;
            }

            Session session = info.getCurrentSession();
            if (session == null){
                result = OperationResult.buildFailureResult("缓存中没有currentSession");
                break;
            }
            ...
            //已经登录了
            result = OperationResult.buildSuccessResult(user.getNickname());
        } while (false);
        return new ResponseEntity(result, HttpStatus.OK);
    }
